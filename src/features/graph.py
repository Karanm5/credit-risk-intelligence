...
class TransactionGraphBuilder:
    ...
    def build_graph(
        ...
        edge_data = (
            transactions.groupby([customer_col, merchant_col])
            .agg({amount_col: ["sum", "mean", "count"], timestamp_col: ["min", "max"]})
            .reset_index()
        )
    ...
    def get_subgraph_for_customer(self, customer_id: str, depth: int = 2) -> nx.Graph:
        """
...
class GraphFeatureEngineer:
    ...
    def compute_all_features(
        ...
        # Merge all features
        result = (
            centrality_features.merge(community_features, on=customer_col)
            .merge(structural_features, on=customer_col)
            .merge(risk_features, on=customer_col)
        )

        return result

    def _compute_centrality_features(
        self, graph: nx.Graph, customers: np.ndarray
    ) -> pd.DataFrame:
    ...
    def _compute_community_features(
        ...
        try:
            communities = nx.community.louvain_communities(
                graph, resolution=self.config.community_resolution, seed=42
            )
    ...
    def _compute_risk_propagation(
        ...
        for customer in customers:
            ...
            if edge_weights.sum() > 0:
                ...
                weighted_avg_risk = (
                    merchant_risks.mean() if len(merchant_risks) > 0 else 0.0
                )
            ...
            results.append(
                {
                    ...
                    "max_merchant_risk": (
                        merchant_risks.max() if len(merchant_risks) > 0 else 0.0
                    ),
