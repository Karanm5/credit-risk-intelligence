...
class TrainingPipeline:
    ...
    def run(self) -> ModelMetrics:
        ...
        with mlflow.start_run(run_name=f"training_pipeline_{run_id}"):
            ...
            logger.info(f"Loaded {len(X_train)} samples with {X_train.shape[1]} features")
    ...
    def _get_feature_list(self, feature_groups: list) -> list:
        ...
        feature_mapping = {
            ...
            "amount": [
                "avg_amount_24h", "avg_amount_7d", "avg_amount_30d",
                "max_amount_7d", "max_amount_30d", "std_amount_7d",
                "std_amount_30d", "total_amount_30d"
            ],
            "velocity": [
                "txn_velocity_change_7d", "txn_velocity_change_30d",
                "amount_velocity_change_7d"
            ],
            "volatility": [
                "spending_cv_7d", "spending_cv_30d", "amount_zscore"
            ],
            "diversity": [
                "unique_merchants_7d", "unique_merchants_30d",
                "unique_categories_30d", "merchant_concentration_30d"
            ],
            "pattern": [
                "weekend_ratio_7d", "weekend_ratio_30d",
                "night_ratio_7d", "night_ratio_30d"
            ],
            "graph": [
                "pagerank_score", "degree_centrality", "weighted_degree",
                "betweenness_centrality", "community_size", "community_density",
                "clustering_coefficient", "num_merchants", "avg_neighbor_degree"
            ],
            "risk_propagation": [
                "merchant_risk_exposure", "high_risk_merchant_ratio",
                "max_merchant_risk", "weighted_avg_merchant_risk",
                "peer_risk_exposure", "high_risk_peer_ratio"
            ]
        }
    ...
    def _check_thresholds(self, metrics: ModelMetrics) -> None:
        ...
        if metrics.auc_roc < thresholds.get("auc_roc", 0):
            logger.warning(f"AUC-ROC {metrics.auc_roc:.4f} below threshold {thresholds['auc_roc']}")
            passed = False
        
        if metrics.ks_statistic < thresholds.get("ks_statistic", 0):
            logger.warning(f"KS Statistic {metrics.ks_statistic:.4f} below threshold {thresholds['ks_statistic']}")
            passed = False
        
        if metrics.gini_coefficient < thresholds.get("gini_coefficient", 0):
            logger.warning(f"Gini {metrics.gini_coefficient:.4f} below threshold {thresholds['gini_coefficient']}")
            passed = False
